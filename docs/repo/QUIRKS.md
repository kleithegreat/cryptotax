...
As the docs hierarchy matures, repository-wide quirks should stay minimal. Source-specific gotchas should live in their own domain `QUIRKS.md` files. Current domain quirks coverage: `docs/evm/QUIRKS.md` (Etherscan spam symbols, zero-amount artifacts), `docs/solana/QUIRKS.md` (Helius mint-as-asset, exact-decimal amounts, ATA signature enumeration, net-flow swaps, fee-payer attribution, endpoint fallback), `docs/hyperliquid/QUIRKS.md` (all-zero funding hash, stale fixtures, `@N` index resolution, partial fill sharing, per-endpoint pagination caps, missing deposits/withdrawals).
