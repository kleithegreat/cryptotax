...
| `hyperliquid.funding_evidence_and_expense_semantics_upgrade` | `blocked` | Upgrade Hyperliquid funding handling: stronger evidence linkage, market context preservation, and potential lot consumption. Core now has structured expense output. | `docs.review.hyperliquid`, `docs.review.core` | `verification.expand_human_verified_exemplars` | downstream Hyperliquid support-claim updates | `docs/hyperliquid/SPEC.md`, `docs/hyperliquid/ARCHITECTURE.md`, `docs/core/SPEC.md`, `docs/core/ARCHITECTURE.md`, `docs/known-transactions.md` |
| `hyperliquid.perp_semantics_upgrade` | `blocked` | Further perp improvements: partial-fill consolidation, 8949 representation, position tracking. Core now quarantines perps and uses ClosedPnl. | `docs.review.hyperliquid` | `verification.expand_human_verified_exemplars` | downstream Hyperliquid support-claim updates | `docs/hyperliquid/SPEC.md`, `docs/hyperliquid/ARCHITECTURE.md`, `docs/known-transactions.md` |
...
| `core.accounting_support_upgrade` | `done` | Implementation checkpoint completed: IR carries additive `asset_canonical` metadata (Option B), perps are quarantined from spot FIFO with `ClosedPnl`-based `perp_pnl.csv` output (Option C), and negative funding writes `funding_expenses.csv` without USDC lot consumption (Option C). Canonical-aware downstream adoption and evidence-backed support claims remain separate tasks. 23 regression tests pass. | `docs.review.core` | - | downstream support-boundary cleanup once evidence is verified | `docs/core/SPEC.md`, `docs/core/ARCHITECTURE.md`, `docs/ir/SPEC.md`, `haskell/src/Types.hs`, `haskell/src/GainLoss.hs`, `haskell/src/Report.hs`, `haskell/test/Spec.hs` |
